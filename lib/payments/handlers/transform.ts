...
export function isTransformProductType(
  productType: unknown
): productType is "transform_starter" | "transform_topup" {
  return productType === "transform_starter" || productType === "transform_topup"
}

export async function handleTransformCheckout(
  ...
  if (!isTransformProductType(productType)) {
    throw new Error(`Unknown Transform product type: ${String(productType)}`)
  }
  if (!isPaymentPaid) {
    console.log(
    ...
    if (!shouldFulfillStripePurchaseCredits(event.livemode)) {
      console.log(
        "[v0] ⏭️ Recorded test-mode Transform payment without mutating production credits"
      )
      return
    }
    ...
    const creditResult = await grantReferencedPurchaseCredits({
      userId,
      amount: credits,
      description: `${productType === "transform_topup" ? "Transform top-up" : "Transform starter pack"} (${credits} credits)`,
      paymentReference: paymentIdForTransform,
      grantPurpose: productType,
      isTestMode,
    })
    ...
    if (!creditResult.success) {
      ...
      await markEventFailed(
        "stripe",
        event.id,
        new Error(`Failed to grant Transform credits: ${creditResult.error}`)
      ).catch(statusError => {
