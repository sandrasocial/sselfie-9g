...
import {
  ...
  persistCheckoutAttributionContact,
...
async function recordCheckoutSessionRevenue(params: {
  ...
  const metadata = {
    ...
    customer_email:
      params.customerEmail || session.customer_details?.email || session.customer_email || null,
...
export async function handleCheckoutSessionCompleted(
  ...
  if (session.mode === "payment") {
    ...
    const customerEmail =
      ...
      null
    const source = resolveCheckoutSource(metadata, productType)
    ...
    if (!customerEmail) {
      console.error(
        `[v0] 🚨 CRITICAL: No customer email found for payment session ${session.id}. Flagging for admin review.`
      )
      await flagForReview({
    ...
    if (isBrandEngineCheckoutProductType(productType)) {
      ...
      const applicationIdRaw = metadata.brand_engine_application_id || metadata.application_id
    ...
    if (!isPaymentPaid) {
      ...
      await handleOneTimeSessionCheckout({
        event,
        session,
        isPaymentPaid,
        customerEmail,
      ...
      await handleSelfieGuideCheckout({
        event,
        session,
        isPaymentPaid,
        customerEmail,
      ...
      await handleStarterKitCheckout({
        event,
        session,
        isPaymentPaid,
        customerEmail,
      ...
      await handleMasterclassCheckout({
        event,
        session,
        isPaymentPaid,
        customerEmail,
      ...
      await handleWorkWithMeCheckout({
        event,
        session,
        isPaymentPaid,
        customerEmail,
      ...
      const blueprintResult = await handlePaidBlueprintCheckout({
        event,
        session,
        isPaymentPaid,
        customerEmail,
