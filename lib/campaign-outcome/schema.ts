...
export async function ensureCampaignOutcomeSchema(): Promise<void> {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS campaign_orders (
      ...
      status TEXT NOT NULL DEFAULT 'awaiting_intake'
        CHECK (status IN (
          'awaiting_intake', 'inputs_ready', 'generating', 'needs_qa',
          'delivered', 'generation_failed', 'refunded'
        )),
      selfie_url TEXT,
      what_she_sells TEXT,
      promotion TEXT,
  ...
  await sql`
    ...
    ADD COLUMN IF NOT EXISTS intake_email_sent_at TIMESTAMPTZ
  `
  await sql`
    ALTER TABLE campaign_orders
