...
const REFUND_RE =
  /\b(refund|chargeback|dispute|cancel|scam|angry|not working|doesn't work|didn't work|broken|complaint|paid but|access issue)\b/i
...
const FAN_RE =
  /\b(love|obsessed|amazing|beautiful|thank you|thanks|inspired|this is so good|need this)\b/i
...
const BARE_KEYWORD_TRIGGERS: Record<string, string> = {
  android: "android_keyword",
  kit: "kit_keyword",
  preset: "preset_keyword",
  presets: "preset_keyword",
  selfie: "selfie_keyword",
  suite: "suite_keyword",
...
const AESTHETIC_PATTERNS: Array<[RegExp, string]> = [
  [
    /\b(dark feminine|dark femme|dark balcony|moody|cinematic|luxury|glam)\b/i,
    "aesthetic_dark_feminine",
  ],
...
export function detectGrowthTags(message: string): string[] {
  ...
  addTag(
    tags,
    /\b(how|chatgpt|upload|use it|do i paste|same chat|new chat)\b/i.test(message),
    "how_to_use_chatgpt"
  )
  addTag(
    tags,
    /\b(face|same face|accurate|looks like me|doesn't look like me|features)\b/i.test(message),
    "face_accuracy_question"
  )
  addTag(tags, /\b(price|cost|how much|expensive|afford)\b/i.test(message), "price_objection")
  addTag(
    tags,
    /\b(done for you|can you do it|make it for me|custom)\b/i.test(message),
    "wants_done_for_you"
  )
  addTag(
    tags,
    /\b(buy|purchase|checkout|pay|order|want in|get the vault)\b/i.test(message),
    "buyer_intent"
  )
...
export function triageIncomingMessage(
  ...
  if (VAULT_RE.test(normalized)) {
    return {
      action: "auto_respond",
      reason: "vault_keyword",
      intent: "vault_interest",
      growthTags,
      confidenceHint: 0.9,
    }
  }
  ...
  if (SELFIE_RE.test(normalized)) {
    return {
      action: "auto_respond",
      reason: "selfie_keyword",
      intent: "selfie_education",
      growthTags,
      confidenceHint: 0.85,
    }
  }
