...
// FUNNEL-EMAIL-01: day-11 of the AI-prompts -> $27 Vault sequence. Light why-now (no fake
// scarcity), the third and final $27 touch before the SUITE trial moves in at day 14.
...
export function generateAiPromptsDay11PromptVaultWhyNowEmail({ firstName, recipientEmail }: AiPromptsDay11Params): {
  html: string
  text: string
  subject: string
} {
  ...
  const bodyHtml = `
    ...
    <div style="margin:26px 0 6px;">${renderStoneButton("Get the Vault · $27", promptVaultUrl)}</div>
  `

  const html = renderStoneShell({
    title: "Still sitting in your camera roll?",
    eyebrow: "Prompt Vault",
    subtitle: "The right moment doesn't come. You just start.",
    bodyHtml,
