...
Sandra`
  ...
  const ctaText = passwordSetupUrl ? "Set your password" : "Open the SUITE"

  const bodyHtml = `
    <p style="margin:0 0 16px;font-size:16px;line-height:1.75;">Hey ${name},</p>
    <p style="margin:0 0 16px;font-size:16px;line-height:1.75;">You're officially in.</p>
    <p style="margin:0 0 16px;font-size:16px;line-height:1.75;">${isMembership ? "The SUITE is ready for you now." : "Your account is ready now."} The fastest way to make this feel real is to get one result on the board today.</p>
    ${renderStonePanel(
    ...
    ${
      passwordSetupUrl
        ? renderStonePanel(
            `<p style="margin:0;font-size:15px;line-height:1.8;color:#a8a49c;">Set your password first, then you'll land inside the SUITE and can start creating straight away.</p>`,
            "First Step",
          )
        : ""
    }
  ...
  const html = renderStoneShell({
    ...
    eyebrow: isMembership ? "SSELFIE SUITE" : "Purchase Confirmed",
    subtitle: "Let's make your first result happen fast.",
    bodyHtml,
  })

  const text = `${isMembership ? "SSELFIE SUITE" : "Purchase Confirmed"}

Hey ${name},

You're officially in.

${isMembership ? "The SUITE is ready for you now." : "Your account is ready now."} The fastest way to make this feel real is to get one result on the board today.

Your order:
${packageName}
${creditsGranted} credits included

${passwordSetupUrl ? `Set your password: ${passwordSetupUrl}` : `Open the SUITE: ${studioUrl}`}
