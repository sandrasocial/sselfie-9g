...
describe("tutorial partnership order form generator", () => {
  ...
  it("locks one evidence-backed tutorial offer and hard-excludes Unlocked Foundation", () => {
    ...
    expect(commandBoard).toContain("Current viable warm offer face value: **EUR 12,000**")
    expect(commandBoard).toContain("no failure notice or reply")
    expect(commandBoard).toContain("Buyer-confirmed fixed-fee offer | USD 160")
