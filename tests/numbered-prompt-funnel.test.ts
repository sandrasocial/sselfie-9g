...
describe("numbered prompt funnel", () => {
  ...
  it("renders the single prompt page through the shared prompt lookup", () => {
    ...
    expect(page).toContain("collectionName={prompt.sourceCollection}")
    expect(page).toContain("lockedShots={prompt.lockedShots}")
    expect(page).toContain("shotCount={prompt.shotCount}")
    ...
    expect(gate).toContain("showAfterCopyVault")
    expect(gate).toContain("You have the first photo. Want the rest of the shoot?")
    expect(gate).toContain("Get every complete shoot · $37")
    expect(gate).toContain("<ShootPreviewStrip")
    expect(gate).toContain("lockedShots={lockedShots}")
    expect(gate).toContain("collectionName={collectionName}")
    expect(gate).toContain("shotCount={shotCount}")
    expect(gate).toContain('cta_position: "after_prompt_copy"')
    ...
    expect(singlePromptEmail).toContain("Open my prompt")
    expect(singlePromptEmail).toContain("tap Copy prompt")
    expect(singlePromptEmail).not.toContain("the Vault is the next step")
