...
describe("Work With Me admin pipeline", () => {
  ...
  it("keeps checkout creation attended and does not send the link automatically", () => {
    ...
    expect(pipeline).toContain("onClick={() => createCheckout(application)}")
    expect(pipeline).toContain("buildWorkWithMeOfferDraft")
    expect(pipeline).toContain("Nothing sends from this page")
