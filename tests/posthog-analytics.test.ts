...
describe("PostHog analytics boundary", () => {
  ...
  it("keeps browser replay and network capture privacy controls enabled", () => {
    ...
    expect(provider).toContain("postHogIdentifiedRotationEpoch = identity.rotationEpoch")
    expect(provider).toContain("isAnalyticsRotationEpochCurrent(result.rotationEpoch)")
    expect(provider).toContain("isAnalyticsRotationEpochCurrent(identity.rotationEpoch)")
    ...
    expect(provider).toContain("setIdentityGeneration(identityGenerationRef.current)")
    expect(provider).toContain("const [identityGeneration, setIdentityGeneration] = useState(0)")
