...
describe("Calendar direct manipulation", () => {
  ...
  it("uses a face-safe focal point for square grid previews", () => {
    ...
    const singlePost = read("components/feed-planner/feed-single-placeholder.tsx")
    const textStudio = read("components/feed-planner/calendar-text-studio.tsx")
    const previewModal = read("components/feed-planner/feed-preview-prompts-modal.tsx")
    const gallery = read("components/feed-planner/feed-gallery-selector.tsx")

    expect(gridItem).toContain("object-top")
    expect(week).toContain("object-top")
    expect(singlePost).toContain("object-top")
    expect(textStudio).toContain("object-top")
    expect(previewModal).toContain("object-top")
    expect(gallery).not.toContain('isPost ? "object-cover object-top" : "object-cover"')
    expect(gridItem).not.toContain("object-[center_20%]")
    expect(week).not.toContain("object-[center_20%]")
  })
})
