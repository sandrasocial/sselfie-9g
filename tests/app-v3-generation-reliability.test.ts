...
describe("Maya style chooser slice", () => {
  it("offers inspiration-image and Maya-decides paths inside the inline style picker", () => {
    ...
    // Maya recommends the strongest real Vault world when the member does not want to browse.
    expect(inline).toContain("Get Maya&apos;s recommendation")
    expect(inline).toContain("Maya recommends the strongest SSELFIE look")
