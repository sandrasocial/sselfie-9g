...
describe("SUITE trial first-image activation", () => {
  ...
  it("shows the first-run selfie step only for trials with zero generated images", () => {
    ...
    expect(concierge).toContain("For stronger likeness")
    expect(concierge).toContain("side profile, and full-body shot")
    expect(concierge).toContain("Inspiration is separate")
  })
})
