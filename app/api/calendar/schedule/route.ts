...
export async function POST(req: NextRequest) {
  try {
    ...
    const { postId, scheduledAt, scheduledTime, contentPillar, timezone } = body

    if (!postId || !scheduledAt) {
      return Response.json({ error: "postId and scheduledAt are required" }, { status: 400 })
    }
    ...
    const updatedPosts = await sql`
      ...
      SET 
        ...
        scheduled_time = ${scheduledTime || "9:00 AM"},
    ...
    return Response.json({ error: "Failed to schedule post", details: error.message }, { status: 500 })
  }
}
