...
export async function GET() {
  ...
  // The Create surface labels the newest live Vault release, not a legacy date rotation.
  const newestCollection = published[0] ?? null
  const newestAesthetic = newestCollection
    ? aesthetics.find(aesthetic => aesthetic.id === toAestheticId(newestCollection.title))
    : null

  return NextResponse.json({
    aesthetics,
    weeklyLook:
      newestCollection && newestAesthetic
        ? {
            aestheticId: newestAesthetic.id,
            name: toDisplayName(newestCollection.title),
            oneLiner: newestCollection.moodLine,
          }
        : null,
  })
}
