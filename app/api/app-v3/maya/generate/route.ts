import { reviewCarouselSlide, type CarouselReview } from "@/lib/app-v3/maya/carousel-review"
import {
...
type AppGraphicRedesignJob = {
  label: string
  carouselOutput?: CreativePlanOutput
...
function buildAppGraphicRedesignJobs({
  ...
  const planned = format === "carousel" ? brief.graphic?.creativePlan?.outputs : undefined
  const slides: CarouselSlide[] = planned?.length
    ? planned.map((output, index) => ({
        kind: index === 0 ? "hook" : index === planned.length - 1 ? "cta" : "photo",
        title: output.title,
        body: output.body,
        purpose: output.purpose,
        visualConcept: output.visualConcept,
        imagePromptDirection: output.imagePromptDirection,
        referenceImageStrategy: output.referenceImageStrategy,
        textSafeArea: output.textSafeArea,
      }))
    : buildGraphicRedesignSlides(brief, format, conceptTitle)
  return slides.map((slide, index) => {
    ...
    return {
      ...
      carouselOutput:
        format === "carousel"
          ? (brief.graphic?.creativePlan?.outputs[index] ?? {
              title: slide.title,
              body: slide.body,
              purpose: slide.purpose ?? "Tell this part of the story",
              visualConcept: slide.visualConcept ?? slide.title,
              imagePromptDirection: slide.imagePromptDirection,
              referenceImageStrategy: "selfie_identity_anchor",
              reasonThisMatchesUserIntent: slide.visualReason ?? topic,
            })
          : undefined,
      visualDirection: brief.graphic?.creativePlan?.visualDirection,
      identityUrls: referenceUrls,
      slide,
...
function normalizeBrief(brief: unknown): CreativeBrief | null {
  ...
  return {
    captureStyle: VALID_CAPTURE_STYLES.has(captureStyle)
      ...
      : undefined,
    polishLevel: VALID_POLISH_LEVELS.has(polishLevel) ? (polishLevel as PolishLevel) : undefined,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const requestedTextOverlayMode =
      normalizeGraphicTextMode(body.textOverlayMode) ??
      (format === "carousel" || requestedOverlayStyle ? "with-text" : null)
    ...
    if (baseImageUrl) {
      ...
      if (
        (format !== "carousel" ||
          brief.graphic?.creativePlan?.outputs.some(o =>
            ["selfie_identity_anchor", "selfie_plus_body_reference"].includes(
              o.referenceImageStrategy
            )
          )) &&
        (typeof referenceSelfieUrl !== "string" || !isAllowedReferenceUrl(referenceSelfieUrl))
      ) {
        return NextResponse.json(
      ...
      if (format === "photoshoot") {
        ...
        if (format !== "carousel" && !graphicStyle) {
          return NextResponse.json(
    ...
    if (format === "carousel") {
      if (graphicJobs.length > 12)
        return NextResponse.json({ error: "Use at most 12 slides" }, { status: 400 })
      for (const job of graphicJobs) {
        if (!job.carouselOutput)
          return NextResponse.json(
            { error: "A complete carousel plan is required" },
            { status: 400 }
          )
        const output = job.carouselOutput
        if (output.title.length > 120 || (output.body?.length ?? 0) > 160)
          return NextResponse.json(
            { error: "Shorten this slide copy or move supporting points into items" },
            { status: 400 }
          )
        if (
          ["screenshot_preserve_exact", "existing_generated_image"].includes(
            output.referenceImageStrategy
          ) &&
          !output.sourceAssets?.length
        ) {
          return NextResponse.json(
            { error: "Attach the original image for this slide before creating it" },
            { status: 400 }
          )
        }
        if ((output.sourceAssets?.length ?? 0) > 3 || (output.items?.length ?? 0) > 6)
          return NextResponse.json(
            { error: "Too many assets or lines on one slide" },
            { status: 400 }
          )
        for (const asset of output.sourceAssets ?? []) {
          if (!isCarouselAssetUrl(asset.url))
            return NextResponse.json({ error: "Invalid carousel asset" }, { status: 400 })
          const owned =
            await sql`SELECT image_url FROM user_avatar_images WHERE user_id = ${String(neonUser.id)} AND image_url = ${asset.url}
            UNION ALL SELECT image_url FROM ai_images WHERE user_id = ${neonUser.id} AND image_url = ${asset.url} LIMIT 1`
          if (!owned.length)
            return NextResponse.json(
              { error: "Carousel source image not found in your uploads" },
              { status: 403 }
            )
        }
        if (job.textOverlaySpec) {
          job.textOverlaySpec = {
            ...job.textOverlaySpec,
            headline: output.title,
            subline: output.body,
            preserveAssets: Boolean(output.sourceAssets?.length),
            items: output.items,
            layout: output.layout,
            position:
              output.textSafeArea === "top" || output.textSafeArea === "upper_third"
                ? "top"
                : output.textSafeArea === "center"
                  ? "center"
                  : "bottom",
          }
        }
        if (textOverlayEnabled && job.textOverlaySpec) {
          try {
            const blank = await sharp({
              ...
              .png()
              .toBuffer()
            await composeCarouselText(blank, job.textOverlaySpec)
          } catch {
            return NextResponse.json(
              {
                error: `Slide ${graphicJobs.indexOf(job) + 1} has too much copy for this layout. Ask Maya to shorten it or split it.`,
              },
              { status: 400 }
            )
          }
        ...
        .then(({ logAnalyticsEvent }) =>
          logAnalyticsEvent({
            ...
            properties: {
              ...
              vaultCollectionId: creativeBriefs[0]?.vaultCollectionId ?? body.aestheticId ?? null,
    ...
    try {
      if (graphicJobs.length > 0) {
        if (format !== "carousel" && !graphicStyle)
          throw new Error("Missing style reference for graphic generation")
        ...
        const renderGraphicJob = async (
          ...
          if (format === "carousel" && job.carouselOutput) {
            const result = await renderCarouselBackground({
              output: job.carouselOutput,
              visualDirection: job.visualDirection ?? "",
              identityUrls: job.identityUrls ?? [],
              quality: IMAGE_QUALITY,
              likeness: likenessBlock,
            })
            actualPromptRecords[index] = result.prompt
    ...
    const wantsAutoBakeBase =
      (format === "carousel" || (body as { autoBake?: unknown }).autoBake === true) &&
      textOverlayEnabled &&
      graphicJobs.length > 0 &&
    ...
    if (format === "carousel" && wantsAutoBakeBase) {
      bakedImageUrls = new Array(buffers.length).fill(null)
      bakedAiImageIds = new Array(buffers.length).fill(null)
      await Promise.all(
        buffers.map(async (buffer, index) => {
          try {
            let composed: Buffer
            try {
              composed = await composeCarouselText(buffer, textOverlaySpecs[index])
            } catch {
              textOverlaySpecs[index] = { ...textOverlaySpecs[index], size: "s" }
              composed = await composeCarouselText(buffer, textOverlaySpecs[index])
            }
            const review =
              Date.now() - requestStartedAt < 260000
                ? await reviewCarouselSlide(
                    composed,
                    textOverlaySpecs[index],
                    index + 1,
                    String(neonUser.id)
                  )
                : {
                    ...
                    issues: ["Please review this slide; automatic checking ran out of time."],
                  }
            carouselReviews.push(review)
            const blob = await put(
              `maya-app-v3/${neonUser.id}/carousel-text-${Date.now()}-${index}.png`,
              composed,
            ...
            VALUES (${neonUser.id}, ${blob.url}, ${imageTitle}, ${persisted[index].id}, ${textOverlaySpecs[index].headline}, ${JSON.stringify(textOverlaySpecs[index])}, 'completed', 'openai', 'carousel', NOW()) RETURNING id`
            bakedImageUrls![index] = blob.url
            bakedAiImageIds![index] = rows[0]?.id ?? null
          } catch (error) {
            console.error("[maya carousel] local composition failed", error)
            autoBakeSkipped = "composition_failed"
            carouselReviews.push({
              slide: index + 1,
              status: "needs_review",
              issues: [
                "Text could not fit on this slide. Ask Maya to shorten the copy or split the slide.",
              ],
            })
          }
        })
      )
    } else if (wantsAutoBake) {
      ...
      carouselReviews.push(
        ...(await Promise.all(
          buffers.map((buffer, index) =>
            Date.now() - requestStartedAt < 260000
              ? reviewCarouselSlide(buffer, undefined, index + 1, String(neonUser.id))
              : Promise.resolve({
                  slide: index + 1,
                  status: "unavailable" as const,
                  issues: ["Please review this slide; automatic checking ran out of time."],
    ...
    return NextResponse.json({
      ...
      ...(carouselReviews.length
        ? { carouselReviews: carouselReviews.sort((a, b) => a.slide - b.slide) }
        : {}),
