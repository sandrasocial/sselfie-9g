...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!code || !userId) {
      return NextResponse.redirect(`${baseUrl}/admin/agent?error=instagram_auth_failed`)
    }
    ...
    if (tokenData.error) {
      ...
      return NextResponse.redirect(`${baseUrl}/admin/agent?error=token_exchange_failed`)
    }
    ...
    if (longLivedData.error) {
      ...
      return NextResponse.redirect(`${baseUrl}/admin/agent?error=long_lived_token_failed`)
    }
    ...
    if (!pagesData.data || pagesData.data.length === 0) {
      ...
      return NextResponse.redirect(`${baseUrl}/admin/agent?error=no_facebook_page`)
    }
    ...
    if (!igData.instagram_business_account) {
      ...
      return NextResponse.redirect(`${baseUrl}/admin/agent?error=no_instagram_account`)
    }
    ...
    return NextResponse.redirect(`${baseUrl}/admin/agent?instagram_connected=true`)
  } catch (error) {
