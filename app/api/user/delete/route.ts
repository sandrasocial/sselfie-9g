...
export async function DELETE() {
  try {
    ...
    await sql`DELETE FROM user_style_guide WHERE user_id = ${userId}`
    await sql`DELETE FROM user_profiles WHERE user_id = ${userId}`

    // Feed planner data
    await sql`DELETE FROM feed_posts WHERE feed_id IN (SELECT id FROM feeds WHERE user_id = ${userId})`.catch(() => null)
    await sql`DELETE FROM feeds WHERE user_id = ${userId}`.catch(() => null)
    await sql`DELETE FROM feed_strategies WHERE user_id = ${userId}`.catch(() => null)

    // Credit transactions
    await sql`DELETE FROM credit_transactions WHERE user_id = ${userId}`.catch(() => null)

    // Subscriptions row
    await sql`DELETE FROM subscriptions WHERE user_id = ${userId}`.catch(() => null)

    // Agent profiles / brand strategy
    await sql`DELETE FROM agent_profiles WHERE user_id = ${userId}`.catch(() => null)
    await sql`DELETE FROM freebie_brand_strategies WHERE user_id = ${userId}`.catch(() => null)

    // Settings
    await sql`DELETE FROM user_settings WHERE user_id = ${userId}`.catch(() => null)
