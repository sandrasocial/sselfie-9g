      ...
      case "checkout.session.completed": {
        ...
        if (session.mode === "payment") {
          ...
          if (productType === "sselfie_studio_membership" && session.mode === "subscription") {
            ...
            console.log(
              `[v0] One-time session purchase for user ${userId} (test mode: ${!event.livemode})`
            )
            ...
            if (paymentIntentId) {
              try {
                ...
                console.log(
                  `[v0] Retrieved payment amount: $${(paymentAmountCents / 100).toFixed(2)}`
                )
              } catch (piError: any) {
            ...
            if (paymentIntentId && paymentAmountCents && customerId) {
              try {
                ...
                console.error(
                  `[v0] Error storing payment in stripe_payments:`,
                  paymentError.message
                )
            ...
            if (process.env.RESEND_PHOTOSHOOT_BUYERS_SEGMENT_ID && customerEmail) {
              try {
                console.log(
                  `[v0] Checking if user ${userId} has Studio membership for segment automation...`
                )
                ...
                if (!hasActiveMembership) {
                  ...
                  if (segmentResult.success) {
                    console.log(
                      `[v0] ✅ Added ${customerEmail} to Instagram Photoshoot Buyers segment`
                    )
                  } else {
                    console.error(
                ...
                console.error(
                  `[v0] ⚠️ Error in Photoshoot Buyers segment automation:`,
                  segmentError
                )
            ...
            if (!isPaymentPaid) {
              ...
              console.log(
                `[v0]     payment_status === "paid": ${session.payment_status === "paid"}`
              )
              console.log(
              ...
              console.log(
                `[v0] 💎 Paid Blueprint purchase from ${customerEmail} - Payment confirmed`
              )
              ...
              if (userId) {
                console.log(
                  `[v0] Using user_id from session.metadata (authenticated checkout): ${userId}`
                )
              ...
              if (!userId && isPaymentPaid) {
                console.error(
                  `[v0] ❌ CRITICAL: Cannot resolve user_id for paid blueprint purchase`,
                  {
                    customerEmail,
                    sessionId: session.id,
                    paymentIntentId,
                ...
                return NextResponse.json(
                  {
                    ...
                    message:
                      "Payment succeeded but user_id could not be resolved. Will retry via cron job.",
                  },
                  { status: 200 }
                )
              }
                ...
                try {
                  ...
                  if (paymentIdForCredits) {
                    ...
                    if (existingCredit.length > 0) {
                      console.log(
                        `[v0] ⏭️ Credits already granted for payment ${paymentIdForCredits} - skipping (idempotency)`
                      )
                    } else {
                      ...
                      if (creditResult.success) {
                        ...
                        console.error(
                          `[v0] ⚠️ Failed to grant paid blueprint credits: ${creditResult.error}`
                        )
                      }
                    }
                  } else {
                    console.error(
                      `[v0] ❌ Missing payment/session identifier for paid blueprint credit grant`,
                      {
                        userId,
                        sessionId: session.id,
                        paymentIntentId,
                  ...
                  console.error(
                    `[v0] ⚠️ Error granting paid blueprint credits (non-critical):`,
                    creditError.message
                  )
              ...
              if (userId && isPaymentPaid) {
                try {
                  ...
                  console.error(
                    `[v0] ⚠️ Error creating subscription entry (non-critical):`,
              ...
              try {
                  ...
                  if (blueprintCheck.length > 0) {
                    ...
                    try {
                      ...
                      if (userFeed && userFeed.length > 0) {
                        ...
                        console.log(
                          `[v0] [FEED EXPANSION] Feed ${feedId} has posts at positions:`,
                          existingPositions
                        )
                        ...
                        if (positionsToCreate.length > 0) {
                          console.log(
                            `[v0] [FEED EXPANSION] Creating posts for positions:`,
                            positionsToCreate
                          )
                    ...
                    `
                    console.log(
                      `[v0] ✅ Created blueprint_subscribers record linked to user ${userId}`
                    )
                  }
                ...
                try {
                  ...
                  if (existingEmail.length > 0) {
                    console.log(
                      `[v0] Skipping duplicate paid-blueprint-delivery email for ${customerEmail}`
                    )
                  } else {
                    // Fetch subscriber data for email (use the accessToken we just set)
                    console.log(
                      `[v0] 🔍 Fetching subscriber data for email delivery: ${customerEmail}`
                    )
                    ...
                    if (subscriber.length > 0 && subscriber[0].access_token) {
                      ...
                      if (emailResult.success) {
                        console.log(
                          `[v0] ✅ Sent paid blueprint delivery email to ${customerEmail}`
                        )
                      } else {
                        console.error(
                          `[v0] ⚠️ Failed to send paid blueprint delivery email: ${emailResult.error}`
                        )
                        // Don't fail webhook - email send failure is non-critical
                      }
                    } else {
                      console.log(
                        `[v0] ⚠️ Subscriber data incomplete (missing access_token) - skipping delivery email for ${customerEmail}`
                      )
                    }
                  }
                } catch (emailError: any) {
                  console.error(
                    `[v0] ⚠️ Error sending paid blueprint delivery email (non-critical):`,
                    emailError.message
                  )
