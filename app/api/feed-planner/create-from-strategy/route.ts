...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[FEED-FROM-STRATEGY] ✅ Posts created. Starting background processing for captions (NO image queueing)...")

    // ==================== BACKGROUND PROCESSING ====================
    // Process captions ONLY in background (fire and forget)
    ...
    processFeedPostsInBackground({
      ...
      .then((result) => {
        if (result.success) {
          console.log(`[FEED-FROM-STRATEGY] ✅ Background processing complete: prompts and captions generated. Images will NOT be auto-queued.`)
        } else {
    ...
    return NextResponse.json({
      ...
      message: hasPromptsInJSON 
        ? "Feed created! Prompts are ready. Captions are being generated in the background. Click 'Generate Feed' to create images."
        : "Feed created! Prompts and captions are being generated in the background. Click 'Generate Feed' to create images.",
      status: "draft", // Feed is ready, waiting for user to generate images
    })
  } catch (error) {
