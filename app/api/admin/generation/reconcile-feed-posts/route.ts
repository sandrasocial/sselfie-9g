...
export async function POST(request: Request) {
  try {
    ...
    const summary = await reconcileFeedPosts({ limit, minAgeMinutes })

    return NextResponse.json({ success: true, summary })
  } catch (error) {
    console.error("[v0] admin reconcile feed posts failed:", error)
    return NextResponse.json(
      { error: "Failed to reconcile feed posts", details: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 },
    )
  }
}
