  ...
  try {
    ...
    for (const post of posts) {
      ...
      if (post.prediction_id && !post.image_url) {
        // Maya Create owns these requests and reconnects them through the Gallery using the
        // same client request id. They are not Replicate prediction ids, so this legacy feed
        // poller must leave them alone while the slot keeps its real server-backed status.
        if (String(post.prediction_id).startsWith("maya:")) continue
        console.log(
          `[v0] [PROGRESS] Checking post ${post.id} (position ${post.position}) with prediction_id: ${post.prediction_id.substring(0, 20)}...`
        )
        ...
        try {
          ...
          if (prediction.status === "succeeded" && prediction.output) {
            ...
            if (!imageUrl || typeof imageUrl !== "string" || imageUrl.trim() === "") {
              console.error(
                `[v0] [PROGRESS] Invalid image URL for post ${post.position}:`,
                imageUrl
              )
              continue
            }
            ...
            try {
              ...
              console.error(
                `[v0] [PROGRESS] ❌ Failed to upload to Blob, using Replicate URL (temporary):`,
                blobError?.message
              )
          ...
          `
            ...
            `
              ...
              `
                ...
                `
                  console.log(
                    `[v0] [PROGRESS] ✅ Image saved to ai_images gallery for post ${post.position}`
                  )
                }
              }
            } catch (galleryError: any) {
              console.error(
                `[v0] [PROGRESS] ❌ Failed to save post ${post.position} to gallery:`,
          ...
          `
            ...
            console.log(
              `[v0] [PROGRESS] ⏳ Post ${post.id} still processing (status: ${prediction.status})`
            )
          }
        } catch (error: any) {
          console.error(
            `[v0] [PROGRESS] ❌ Error checking prediction for post ${post.id}:`,
            error?.message || error
          )
        ...
        if (post.generation_status !== "completed") {
          console.log(
            `[v0] [PROGRESS] ⚠️ Post ${post.id} has image_url but status is ${post.generation_status}, fixing...`
          )
          await sql`
            UPDATE feed_posts
    ...
    console.log(
      `[v0] [PROGRESS] Summary: ${completedCount} completed, ${failedCount} failed, ${posts.length - completedCount - failedCount} still processing`
    )
