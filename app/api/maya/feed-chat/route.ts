...
2. Call the generateCompleteFeed tool (which automatically generates captions)
...
const generateCompleteFeedTool = tool({
  description:
    "Design a complete 9-post Instagram feed strategy with VISUAL CONCEPTS ONLY. This tool creates the visual layout, Flux prompts, and post structure. Bio and captions will be generated by specialists.",
...
Be creative and authentic. No generic templates - every element should feel custom-designed for this specific brand.`,
    ...
    try {
      ...
      console.log("[v0] [SERVER] ✓ All", postsWithoutCaptions.length, "posts saved to database")

      console.log("[v0] [SERVER] 🎯 Step 3: Calling Instagram Caption Strategist directly...")
      try {
      ...
      for (const highlight of highlights) {
        try {
          ...
          if (response.ok) {
            ...
            console.error("[v0] [SERVER] ⚠️ Failed to generate highlight:", highlight.title, "Status:", response.status)
            errors.push(`${highlight.title}: ${response.status}`)
          }
        } catch (error) {
          console.error("[v0] [SERVER] ⚠️ Error generating highlight:", highlight.title, error)
