...
import { SELFIE_AI_PHOTOS_KIT_SOURCE, SELFIE_AI_PHOTOS_KIT_TAG } from "@/lib/freebie/selfie-ai-photos-kit-access"
...
export default async function SelfieAiPhotosKitAccessPage({
  ...
  if (!result.valid) {
    return (
      <main className={`min-h-screen bg-brand-porcelain px-5 py-16 text-brand-obsidian ${inter.className}`}>
        <div className="mx-auto max-w-xl border border-stone-pale/55 bg-white p-8 text-center shadow-2xl">
          ...
          <p className="mb-7 text-sm font-light leading-relaxed text-stone-quarry">
            If you bought the Selfie To AI Photos Kit, use the email from your checkout or recover access below.
          </p>
          <Link href="/access" className="inline-block bg-brand-obsidian px-6 py-3 text-[10px] font-medium uppercase tracking-[0.2em] text-white">
            Recover access
          </Link>
        </div>
      </main>
  ...
  return (
    <main className={`min-h-screen bg-brand-porcelain text-brand-obsidian ${inter.className}`}>
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-5 py-6">
        <Link href="/" className={`text-xl font-light uppercase tracking-[0.26em] ${cormorant.className}`}>
          SSELFIE
        </Link>
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-5 pb-12 pt-6 md:grid-cols-[1fr_0.8fr] md:items-end">
        <div>
          ...
          <h1 className={`max-w-3xl text-5xl font-light leading-[0.95] sm:text-6xl md:text-7xl ${cormorant.className}`}>
        ...
        <div className="grid grid-cols-3 gap-2">
          {STARTER_PROMPTS.map((prompt) => (
            <div key={prompt.title} className="relative aspect-[3/4] overflow-hidden bg-white">
              <Image src={prompt.image} alt="" fill sizes="160px" className="object-cover object-center" />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="border-y border-stone-pale/55 bg-white">
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-10 md:grid-cols-[0.8fr_1fr]">
          ...
          <div className="grid gap-3">
            {CHECKLIST.map((item) => (
              <div key={item} className="border border-stone-pale/45 bg-brand-porcelain px-4 py-3 text-sm font-light leading-relaxed text-stone-quarry">
                {item}
              </div>
            ))}
          </div>
      ...
      <section className="mx-auto max-w-6xl px-5 py-14">
        <div className="mb-8 max-w-2xl">
          ...
          <p className="mt-4 text-sm font-light leading-relaxed text-stone-quarry">
            Upload your selfie to ChatGPT first. Then paste the prompt. If the image is close, fix the detail instead of changing the whole direction.
          </p>
        </div>
        <div className="grid gap-5 md:grid-cols-3">
          {STARTER_PROMPTS.map((prompt) => (
            <article key={prompt.title} className="border border-stone-pale/55 bg-white shadow-xl">
              <div className="relative aspect-[4/5] overflow-hidden bg-brand-porcelain">
                <Image src={prompt.image} alt={`Example direction for ${prompt.title}`} fill sizes="(max-width: 768px) 90vw, 30vw" className="object-cover object-center" />
              </div>
              <div className="p-5">
                <h3 className={`text-3xl font-light leading-none ${cormorant.className}`}>{prompt.title}</h3>
                <p className="mt-3 text-xs font-light leading-relaxed text-stone-quarry">{prompt.use}</p>
                <div className="mt-5 border border-stone-pale/45 bg-brand-porcelain p-4">
                  <p className="text-xs font-light leading-relaxed text-stone-quarry">{prompt.prompt}</p>
                  <div className="mt-4">
                    <CopyButton
                      text={prompt.prompt}
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {FIX_PROMPTS.map((fix) => (
              <article key={fix.title} className="border border-white/15 bg-white/5 p-5">
                ...
                <p className="mt-3 text-xs font-light leading-relaxed text-stone-pale">{fix.prompt}</p>
                <div className="mt-5">
                  <CopyButton
                    text={fix.prompt}
      ...
      <section className="mx-auto grid max-w-6xl gap-4 px-5 py-14 md:grid-cols-2">
        ...
        <div className="border border-stone-pale/55 bg-white p-7 shadow-xl">
          ...
          <p className="mt-4 text-sm font-light leading-relaxed text-stone-quarry">
            Use Maya when you want a monthly system for photos, covers, captions, and content direction.
          </p>
          <SelfieAiKitTrackedLink
            href={suiteUrl}
