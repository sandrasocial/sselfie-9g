...
- **Prompt Vault pivot**: Sandra approved a front-door growth pivot on 2026-05-26. Audience behavior is strongest around AI photoshoot prompts for ChatGPT. The active public path is now `/ai-prompts` free lead magnet -> `/prompt-vault` paid low-ticket offer -> deeper offers. Do not drift back to making Starter Kit the primary upgrade from AI prompts. Starter Kit remains live for existing buyers and as a secondary support product only.
...
- **Prompt Vault launch ops**: `/admin/prompt-vault` is the live launch monitor for visits, checkout starts, purchases, access opens, and prompt copies. The approval-only launch broadcast draft is `docs/email/PROMPT_VAULT_LAUNCH_BROADCAST_DRAFT_2026-05-26.md`.
- **Selfie education reposition**: Approved source of truth is `docs/SELFIE-EDUCATION-REPOSITION-PLAN-2026-04-23.md`, but its Starter Kit-first ladder is superseded for front-door growth. Keep `/selfie-guide`, `/starter-kit`, `/masterclass`, `/join/studio`, `/work-with-me`, `/checkout/starter-kit`, `/checkout/masterclass`, and `/access/starter-kit/[token]` working; do not treat them as the current main funnel unless Sandra explicitly reverses the Prompt Vault pivot.
...
| `docs/PROMPT_VAULT_ADD_COLLECTION_SOP.md` | Source of truth for adding Prompt Vault collections. It locks the rule: free AI prompts show one preview shot per collection; paid Prompt Vault gets the full collection. |
