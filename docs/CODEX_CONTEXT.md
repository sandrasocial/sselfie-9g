...
## Current focus areas (updated 2026-04-23)

- **Selfie education reposition**: Approved source of truth is `docs/SELFIE-EDUCATION-REPOSITION-PLAN-2026-04-23.md`. New ladder is Free Selfie Guide -> Starter Kit -> Masterclass -> Studio -> 1:1. Public routes planned/owned by this reposition include `/selfie-guide`, `/starter-kit`, `/masterclass`, `/join/studio`, `/work-with-me`, `/checkout/starter-kit`, `/checkout/masterclass`, and `/access/starter-kit/[token]`.
- **Lifecycle email update**: `app/api/cron/nurture-sequence/route.ts` now owns lifecycle sends for Free Guide, Starter Kit, Masterclass, and the legacy Brand Strategy follow-up. Free Guide uses `freebie_subscribers` + `/selfie-guide/access/[token]`. Starter Kit uses tokenized access at `/access/starter-kit/[token]`. Masterclass uses Academy-style entitlement delivery plus a dedicated email sequence.
...
| `docs/brand/DESIGN_SYSTEM.md` | **Single visual + product UI contract for SSELFIE.** All design decisions and token usage must align here first. |
