...
| `scripts/` | Diagnostic + onboarding scripts (Resend tests, automation instrumentation). Includes `user-journey-smoke.ts` for headless public funnel validation and `verify-onboarding-segment-env.ts` for real Resend segment drift checks. Run them to double-check behavior before code changes. |
