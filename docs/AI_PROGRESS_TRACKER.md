...
| M-08 | Monetization | 72-hour churn intervention sprint (Day 14/21/28 lifecycle + usage recap) | Codex | In Progress | At-risk member contact coverage >=90% in 72h | 2026-02-18 | `docs/SURVIVAL_TO_GROWTH_EXECUTION_PLAN_2026-02-15.md`, `lib/email/marketing-template-catalog.ts`, `lib/email/templates/welcome-sequence.ts`, `app/api/cron/welcome-sequence/route.ts`, `tests/welcome-sequence-lifecycle.test.ts`, `output/automation/email-performance-*.md` | High | Add monthly usage recap + verify first production runs |
