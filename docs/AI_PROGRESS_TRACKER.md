...
| M-04 | Monetization | Run Cohort launch sprint ops + daily launch digest | Codex + Sandra | Deferred | 3 calls/day pace and seat-fill visibility to March 16 cohort | 2026-02-19 | `app/admin/brand-engine-applications/applications-client.tsx`, `app/api/cron/brand-engine-launch-digest/route.ts`, `scripts/brand-engine-launch-digest.mjs`, `docs/brand/COHORT_LAUNCH_SPRINT_2026-03-16.md`, `vercel.json` | Low | ⛔ DEFERRED 2026-03-02 — Brand Engine cohort pivoted. Focus is Agent V1. Resume only if strategy reverts. |
...
| R-01 | Reliability | Keep cron fleet stable during launch window | Stella | In Progress | 7-day cron success >= 98% | 2026-02-26 | `output/automation/triage-*.md` | High | Monitor and triage daily |
| R-02 | Reliability | Keep lifecycle email failures under threshold | Stella | In Progress | Failures < 2% for active sends | 2026-02-26 | `output/automation/email-performance-*.md` | High | Monitor follow-up edge cases |
...
| V-03 | Voice | Enable draft-only outbound for launch period | Stella + Sandra | Done | 100% outbound launch copy human-approved | 2026-02-20 | `docs/AI_OPERATIONS_EXECUTION_BOARD.md`, `docs/MYNOTES_VOICE_STYLE_INTAKE.md` | Low | Maintain until explicit upgrade |
...
| M-10 | Monetization | Brand Engine launch queue execution (daily SLA on qualified_queue -> closed_won) | Sandra + Codex | Deferred | 3 calls/day pacing and clean stage progression | 2026-03-16 | `app/admin/brand-engine-applications/applications-client.tsx`, `output/automation/brand-engine-launch-digest-*.md` | Low | ⛔ DEFERRED 2026-03-02 — Brand Engine cohort pivoted. Depends on M-04 which is also deferred. |
...
| UX-01 | Design | Design System Foundation: dark tokens, Cormorant+Inter, glass utilities | Codex | Done | pnpm build passes, Vercel preview confirms dark | 2026-03-03 | `app/globals.css`, `lib/design-tokens.ts`, `tailwind.config.ts`, `https://v0-sselfie-h1lfdhk6h-sselfie-studio.vercel.app` | Low | Continue with A-01 then A-02 |
| A-01 | Activation | Maya First-Generation Guided Path: strict bonus/no-image-spend gate + validated onboarding tests | Codex | Done | Guided flow only appears for true first-use cohort and analytics firing remains intact | 2026-02-27 | `lib/onboarding/welcome-first-generation.ts`, `tests/welcome-first-generation.test.ts`, `components/sselfie/maya/welcome-first-generation-flow.tsx`, `app/api/user/credits/route.ts`, `https://v0-sselfie-lj5nxfvik-sselfie-studio.vercel.app` | Medium | Start A-02 wizard simplification |
| A-02 | Activation | Feed Planner wizard simplification: keep 3-step wizard, remove checklist wall, and enforce inline paid-blueprint setup path | Codex | Done | Higher first-feed-start rate from prominent CTA and reduced paid-user setup friction | 2026-02-27 | `app/feed-planner/feed-planner-client.tsx`, `components/feed-planner/feed-view-screen.tsx`, `components/onboarding/unified-onboarding-wizard.tsx`, `https://v0-sselfie-fxp9suc4s-sselfie-studio.vercel.app` | Medium | Start UX-02 redesign |
| UX-02 | Design | Maya Screen: glassmorphic dark UI, 6-tab switcher, Instagram preview cards elevated | Codex | Done | Vercel preview approved by Sandra | 2026-03-05 | `components/sselfie/maya-chat-screen.tsx`, `components/sselfie/maya/maya-header.tsx`, `components/sselfie/maya/maya-tab-switcher.tsx`, `components/sselfie/maya/maya-unified-input.tsx`, `components/sselfie/maya/maya-chat-interface.tsx`, `components/sselfie/concept-card.tsx`, `components/sselfie/instagram-photo-card.tsx`, `components/sselfie/instagram-carousel-card.tsx`, `components/sselfie/instagram-reel-card.tsx`, `components/sselfie/instagram-photo-preview.tsx`, `components/sselfie/instagram-reel-preview.tsx`, `https://v0-sselfie-7433w7q11-sselfie-studio.vercel.app` | Medium | Start UX-03 |
| UX-03 | Design | Academy Landing + Product Pages: editorial redesign, numbered product list, Cormorant hero | Codex | Done | Vercel preview approved | 2026-03-05 | `app/academy/page.tsx`, `app/academy/products/[productId]/page.tsx`, `app/academy/products/[productId]/purchase-button.tsx`, `app/academy/success/page.tsx`, `https://v0-sselfie-mb4h9f6s0-sselfie-studio.vercel.app` | Low | Start UX-04 |
| UX-04 | Design | Gallery: dark grid, 4 correct tabs, glassmorphic lightbox | Codex | Done | Vercel preview approved | 2026-03-07 | `components/sselfie/gallery-screen.tsx`, `components/sselfie/fullscreen-image-modal.tsx`, `6b3b7b43` | Low | Completed |
| UX-05 | Design | Feed Planner: dark glassmorphic, 9-post grid as hero | Codex | Done | Vercel preview approved | 2026-03-07 | `app/feed-planner/feed-planner-client.tsx`, `components/feed-planner/*`, `a2315e9a` | Low | Completed |
| UX-06 | Design | Profile/Account: dark glassmorphic 2-tab | Codex | Done | Vercel preview approved | 2026-03-10 | `components/sselfie/account-screen.tsx`, `components/sselfie/best-work-selector.tsx`, `components/sselfie/personal-brand-section.tsx`, `5243544f` | Low | Completed |
| UX-07 | Design | Academy In-App: 5-view glassmorphic, no icons, mini-products editorial | Codex | Done | Vercel preview approved | 2026-03-10 | `components/sselfie/academy-screen.tsx`, `components/academy/*`, `47a28b9d` | Low | Completed |
| UX-11 | Design | Mounted-surface UX consistency hardening (no icons/emojis in active Studio surfaces) | Stella | Done | 0 `lucide-react` in mounted surface scope + build pass | 2026-03-01 | `docs/codex-tasks/UX-CONSISTENCY-AUDIT-2026-03-01.md`, `components/feed-planner/*`, `components/sselfie/maya/*`, `components/academy/*`, `5b2bb71d` | Low | ✅ Done — shipped and verified 2026-03-02 |
