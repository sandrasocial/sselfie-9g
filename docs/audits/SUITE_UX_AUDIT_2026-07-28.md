...
3. **Auto-open of stale drawer on load + regenerating Learn content (P2).** HEAD already forces `setIsOpen(false)` on both restore branches (`concierge-context.tsx:697, :718`; commit `e2a3445f`) — the live site still auto-opens, so **production is running a pre-fix build; push/deploy resolves this**. Separately, the "Learn with Maya" surface re-calls `POST /api/app-v3/maya/guidance` on every open with no cache and temp 0.2 (`maya-guidance-workspace.tsx:42-76`, `guidance/service.ts:259-267`) — new advice wording every open (observed 3 different versions live), token cost per open, and a latent trap: `continuity.ts:624-645` re-persists server `isOpen` that future code must never trust.
