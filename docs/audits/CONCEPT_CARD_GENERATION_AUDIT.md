...
**🔴 ISSUE #1: Upload Module Override**
...
**🔴 ISSUE #2: Prompt Built from Description**
...
**🔴 ISSUE #3: Massive Post-Processing Modifications**
...
**✅ GOOD:** Pro Mode logs what Maya generates before modifications.
...
**🔴 ISSUE #4: Prompt Built from Description**
...
**🔴 ISSUE #5: Description Enhancement**
...
**🔴 ISSUE #9: No Original Data Display**
...
**🔴 ISSUE #10: Prompt Editor Uses Modified Prompt**
...
**🔴 ISSUE #11: Prompt Rebuilding at Generation Time**
...
**🔴 ISSUE #12: Pro Mode Prompt Rebuilding**
...
1. Maya generates concepts
   ...
   ├─ description: "Cozy holiday morning moment..."
   └─ category: "Lifestyle"

2. Upload Module Override (if applicable)
   ├─ title: "Category - Concept 1" ❌ OVERRIDDEN
   ├─ description: "category concept concept..." ❌ OVERRIDDEN
   └─ category: "category" ❌ OVERRIDDEN

3. Direct Prompt Generation
   ├─ Input: concept.description (may be overridden)
   ├─ Process: generatePromptDirect()
   └─ Output: concept.prompt (structured prompt)
...
4. Post-Processing (Lines 4000-4207)
   ...
   └─ Final cleanup
   └─ Result: Heavily modified prompt

5. Storage
   └─ Saved to database (modified data)

6. Display
   └─ UI shows modified title/description

7. Generation (if user generates image)
   ├─ Quality validation
   ├─ May rebuild prompt if checks fail
   └─ Sent to Replicate
```
...
2. Direct Prompt Generation
   ...
   ├─ Process: generatePromptDirect()
   ├─ Camera spec override (based on conceptIndex)
   └─ Output: concept.prompt (structured prompt)
...
4. Display
   └─ UI shows title/description

5. Generation (if user generates image)
   ├─ buildNanoBananaPrompt() transforms prompt again
   └─ Sent to Nano Banana Pro
```
...
### 🔴 CRITICAL ISSUE #1: Upload Module Override
...
### 🔴 CRITICAL ISSUE #2: Prompt Not Generated by Maya
...
### 🔴 CRITICAL ISSUE #3: Extensive Post-Processing
...
### 🔴 CRITICAL ISSUE #4: Prompt Rebuilding at Generation
