...
const feedHeaderStatClass =
  "rounded-[8px] border border-[#C5C6C8]/60 bg-white px-2 py-1.5 text-center"
        ...
        <div className="mb-3 flex items-start gap-4 md:gap-8">
          ...
          <div className="flex-1 space-y-3">
            ...
            <div className="flex gap-1.5 flex-wrap">
              ...
              {!access?.isFree &&
                (access?.isPaidBlueprint || access?.isMembership) &&
                needsFirstStyleChoice && (
                  <button
              ...
              {!access?.isFree && (
                <button onClick={onWriteBio} className={feedHeaderChipClass}>
                  Bio
                </button>
              )}
              {!access?.isFree && onCreateHighlights && (
                <button onClick={onCreateHighlights} className={feedHeaderChipClass}>
                  Highlights
                </button>
              )}
                ...
                <div className="flex gap-3 md:gap-4 overflow-x-auto pb-2 scrollbar-hide">
                  {feedData.highlights.map((highlight: any) => {
                    ...
                    const highlightColor =
                      displayColor ||
                      availableColors[
                        feedData.highlights.indexOf(highlight) % availableColors.length
                      ]

                    return (
                      <div
                        key={highlight.id || highlight.title}
                        className="flex flex-col items-center gap-2 min-w-[64px] md:min-w-[70px] shrink-0"
                      >
