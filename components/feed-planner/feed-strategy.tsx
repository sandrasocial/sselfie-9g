    ...
    <div className="app-light-panel-text p-3 md:p-6">
      ...
      {feedData.strategy?.posting_schedule && (
        <div className="space-y-4 mt-6">
          <div className="text-xs tracking-[0.2em] uppercase font-medium text-white/55">When To Post</div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {feedData.strategy.posting_schedule.optimalTimes?.map((time: any, idx: number) => (
              <div key={idx} className="space-y-2 rounded-xl border border-white/12 bg-white/[0.04] p-4">
                <div className="text-sm font-medium text-white">{time.day}</div>
                <div className="text-lg font-semibold text-white">{time.time}</div>
                <div className="text-xs text-white/65">{time.reason}</div>
              </div>
            ))}
          </div>
          <div className="rounded-xl border border-white/12 bg-white/[0.04] p-4">
            <div className="mb-2 text-xs text-white/55">Posting Frequency</div>
            <div className="text-sm text-white/80">{feedData.strategy.posting_schedule.frequency}</div>
          </div>
        </div>
      )}
      ...
      {feedData.strategy?.content_pillars && (
        <div className="space-y-4 mt-6">
          <div className="text-xs tracking-[0.2em] uppercase font-medium text-white/55">
            Content Mix Strategy
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {Object.entries(feedData.strategy.content_pillars).map(([key, value]: any) => (
              <div key={key} className="space-y-2 rounded-xl border border-white/12 bg-white/[0.04] p-4 backdrop-blur-xl">
      ...
      {feedData.strategy?.caption_templates && Array.isArray(feedData.strategy.caption_templates) && (
        <div className="space-y-4 mt-6">
          ...
          {feedData.strategy.caption_templates.slice(0, 9).map((story: any, idx: number) => (
            <div key={idx} className="space-y-3 rounded-xl border border-white/12 bg-white/[0.04] p-4 backdrop-blur-xl">
              <div className="flex items-center justify-between">
                <div className="text-sm font-medium text-white">Post {story.postNumber || idx + 1}</div>
                <div className="text-xs text-white/55">{story.storyTiming}</div>
              </div>
              <div className="text-xs text-white/70">{story.storyPurpose}</div>
              <div className="space-y-1.5">
                {story.storySequence?.map((seq: string, i: number) => (
                  <div key={i} className="flex items-start gap-2">
                    <div className="w-5 h-5 rounded-full bg-white/14 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span className="text-[10px] font-semibold text-white/70">{i + 1}</span>
                    </div>
                    <div className="text-xs text-white/80 leading-relaxed">{seq}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {feedData.strategy?.content_format_mix?.reels && (
        <div className="space-y-4 mt-6">
          ...
          {feedData.strategy.content_format_mix.reels.map((reel: any, idx: number) => (
            <div key={idx} className="space-y-3 rounded-xl border border-white/12 bg-white/[0.04] p-4 backdrop-blur-xl">
              <div className="text-sm font-medium text-white">Post {reel.postNumber} → Reel</div>
              <div className="text-xs text-white/70 leading-relaxed">{reel.reelConcept}</div>
              <div className="space-y-2">
                <div className="text-[10px] text-white/55 uppercase tracking-wider">Hook</div>
                <div className="text-xs text-white/80 font-medium">{reel.hookSuggestion}</div>
              </div>
              <div className="space-y-2">
                <div className="text-[10px] text-white/55 uppercase tracking-wider">Trending Audio</div>
                <div className="text-xs text-white/80">{reel.audioRecommendation}</div>
              </div>
              <div className="text-[10px] text-white/55">{reel.coverPhotoTips}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {feedData.strategy?.content_format_mix?.carousels && (
        <div className="space-y-4 mt-6">
          <div className="text-xs tracking-[0.2em] uppercase font-medium text-white/55">Carousel Ideas</div>
          {feedData.strategy.content_format_mix.carousels.map((carousel: any, idx: number) => (
            <div key={idx} className="space-y-3 rounded-xl border border-white/12 bg-white/[0.04] p-4 backdrop-blur-xl">
              <div className="text-sm font-medium text-white">Post {carousel.postNumber} → Carousel</div>
              <div className="text-xs text-white/70 leading-relaxed">{carousel.carouselIdea}</div>
              <div className="space-y-1.5">
                {carousel.slideBreakdown?.map((slide: string, i: number) => (
                  <div key={i} className="flex items-start gap-2">
                    ...
                    <div className="text-xs text-white/80">{slide}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {feedData.strategy?.growth_tactics && (
        <div className="space-y-4 mt-6">
          <div className="text-xs tracking-[0.2em] uppercase font-medium text-white/55">Growth Tactics</div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {Object.entries(feedData.strategy.growth_tactics).map(([key, tactics]: any) => (
              <div key={key} className="space-y-3 rounded-xl border border-white/12 bg-white/[0.04] p-4 backdrop-blur-xl">
                <div className="text-sm font-medium text-white capitalize">
                  {key.replace(/([A-Z])/g, " $1").trim()}
                </div>
                <div className="space-y-1.5">
                  {tactics.map((tactic: string, i: number) => (
                    <div key={i} className="flex items-start gap-2">
                      <div className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-white/45" />
                      <div className="text-xs text-white/70 leading-relaxed">{tactic}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {feedData.strategy?.hashtag_strategy && (
        <div className="space-y-4 mt-6">
          <div className="text-xs tracking-[0.2em] uppercase font-medium text-white/55">Hashtag Strategy</div>
          <div className="space-y-4 rounded-xl border border-white/12 bg-white/[0.04] p-4 backdrop-blur-xl">
            <div>
              <div className="text-xs text-white/55 mb-2">Main Hashtags (Use on every post)</div>
              <div className="flex flex-wrap gap-2">
                {feedData.strategy.hashtag_strategy.mainHashtags?.map((tag: string, i: number) => (
                  <span
                    key={i}
                    className="rounded-full border border-white/12 bg-white/[0.06] px-3 py-1.5 text-xs text-white/80"
                  >
                    {tag}
                  </span>
                ))}
            ...
            <div>
              <div className="text-xs text-white/55 mb-2">Rotating Hashtags (Vary by post)</div>
              <div className="flex flex-wrap gap-2">
                {feedData.strategy.hashtag_strategy.rotatingHashtags?.map((tag: string, i: number) => (
                  <span key={i} className="px-3 py-1.5 bg-white/10 text-white/70 text-xs rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
            </div>
            <div className="border-t border-white/12 pt-2 text-xs text-white/70">
              Tip: {feedData.strategy.hashtag_strategy.hashtagPlacement}
            </div>
          </div>
        </div>
      )}
      ...
      {feedData.strategy?.content_format_mix?.trends && (
        <div className="mt-6 space-y-4 rounded-xl border border-white/12 bg-white/[0.04] p-6 backdrop-blur-xl">
          <div className="text-xs tracking-[0.2em] uppercase font-medium text-white/55">Trend Strategy</div>
          <div className="space-y-3">
            <div>
              <div className="text-xs text-white/55 mb-1">When to Use Trends</div>
              <div className="text-sm text-white/80 leading-relaxed">
                {feedData.strategy.content_format_mix.trends.whenToUseTrends}
              </div>
            </div>
            <div>
              <div className="text-xs text-white/55 mb-1">Trending Audio</div>
              <div className="text-sm text-white/80">
                {feedData.strategy.content_format_mix.trends.trendingAudio?.join(", ")}
              </div>
            </div>
            <div>
              <div className="text-xs text-white/55 mb-1">Brand Alignment</div>
              <div className="text-sm text-white/80 leading-relaxed">
      ...
      {/* Grid Pattern */}
      <div className="mt-6 space-y-3 rounded-xl border border-white/12 bg-white/[0.04] p-4 backdrop-blur-xl">
        <div className="text-xs tracking-[0.2em] uppercase font-medium text-white/55">Grid Pattern</div>
        <div className="text-sm font-light text-white/80">{feedData.feed.layout_type || "Balanced Mix"}</div>
