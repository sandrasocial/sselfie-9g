...
export function CalendarPlanSettingsCard({
  ...
  const { businessType, currentSituation, feedStyle, idealAudience } = settings
  ...
  useEffect(
    () => setDraft({ businessType, currentSituation, feedStyle, idealAudience }),
    [businessType, currentSituation, feedStyle, idealAudience]
  )
  useEffect(() => {
    ...
    >
      ...
      <p className="mt-2 text-[13px] leading-relaxed text-[color:var(--app-text-primary)]">
        {complete
          ...
          : "Tell me these three things so I can plan from your real business instead of guessing."}
      </p>
      ...
      <button
        ...
        className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-[9px] bg-[color:var(--app-btn-primary-bg)] px-4 text-[12px] font-medium text-[color:var(--app-btn-primary-text)] disabled:opacity-40"
      >
        {saving ? <Loader2 className="animate-spin" size={15} aria-hidden /> : null}
        Save content context
      </button>
    </section>
  )
}
