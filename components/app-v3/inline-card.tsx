...
export function InlineCard({ eyebrow, title, subtitle, actions, children, className }: InlineCardProps) {
  const hasHeader = Boolean(eyebrow || title || subtitle)
  return (
    <div className={`overflow-hidden rounded-[8px] border border-[#C5C6C8]/60 bg-white ${className ?? ""}`}>
      <div className="p-5">
        {hasHeader && (
          <div>
            {eyebrow && (
            ...
            {title && (
              <h3 className="mt-2 font-serif text-[22px] font-light leading-tight text-[#0D0E10]">{title}</h3>
            )}
            {subtitle && <p className="mt-2 text-[14px] leading-relaxed text-[#4F5052]">{subtitle}</p>}
          </div>
        )}
