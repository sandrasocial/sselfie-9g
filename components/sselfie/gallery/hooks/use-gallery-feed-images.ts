...
interface UseGalleryFeedImagesReturn {
  images: GalleryImage[]
  isLoading: boolean
  error: any
  hasMore: boolean
...
export function useGalleryFeedImages(): UseGalleryFeedImagesReturn {
  ...
  const getKey = (pageIndex: number, previousPageData: any) => {
    ...
    return `/api/images/feed?limit=50&offset=${pageIndex * 50}`
  }
  ...
  const images: GalleryImage[] = data 
    ? data.flatMap((page) => {
        ...
        if (!Array.isArray(pageImages)) {
          console.error("[Gallery] Feed page images is not an array:", page)
          return []
        }
        return pageImages
      })
