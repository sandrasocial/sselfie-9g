...
export default function FeedDesignerScreen() {
  ...
  const { status, messages, sendMessage, setMessages, append } = useChat({
    ...
    onError: (error) => {
      ...
      setIsApplyingDesign(false) // Ensure this is also reset
      setMayaThinking(null) // Clear thinking state on error
    },
  })
  ...
  useEffect(() => {
    if (!isTyping && messages.length > 0) {
      ...
      if (lastMessage.role === "assistant" && lastMessage.toolInvocations) {
        ...
        if (hasFeedTool) {
          console.log("[v0] Feed generation tool completed, loading latest feed...")
          // Simply load the latest feed when tool completes
          loadLatestFeed()
            .then(() => {
              console.log("[v0] Feed loaded successfully")
              setMayaThinking(null) // Assuming setMayaThinking is intended here
            })
            .catch((error) => {
    ...
    try {
      ...
      if (currentFeedId) {
        ...
        if (deleteResponse.ok) {
          ...
          console.error("[v0] [NEW FEED] Failed to delete feed:", await deleteResponse.text())
        }
      } else {
      ...
      // Set Maya thinking and let her generate the feed
      setMayaThinking("Starting your feed generation...")
      setShowNewFeedModal(false)

      // Send message to Maya to generate the feed
      ...
      // Reset the ref after a delay to prevent rapid re-clicks
      setTimeout(() => {
