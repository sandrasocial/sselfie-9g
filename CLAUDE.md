...
| MAYA-ADMIN-01 — Maya IS the admin content surface (Sandra's call: don't build parallel tools, embed Maya + admin persona). Vault vibe tiles front and center: keep a collection's world, swap outfit/location in chat | **Mostly built** — Slice 1, approve/publish tooling, admin content tools, weekly brief context, and admin-specific editorial memory are live. Remaining: final handoff/live QA polish and optional reel-cover/caption tools after Sandra tests the carousel/story workflow. Spec: `tasks/MAYA-ADMIN-01-admin-maya.md` |
| SHOOT-STUDIO-01 — inspiration-image photoshoots (now a collapsed support tool; engine + vault-anatomy prompt writer feed MAYA-ADMIN-01 slices) | **Phase B/C built 2026-06-13**: shoots generate 6+ shots, publish to DB-backed Vault/freebie/Library/Maya surfaces, and include drop-email preview/test/dry-run/send flow. Remaining work is operational inventory + live QA after at least 2 queued shoots exist. Spec: `tasks/SHOOT-STUDIO-01-admin-shoot-studio.md`. vault-prompt-writer skill recreated + COMMITTED at `.agents/skills/vault-prompt-writer/`. |
