...
| SHOOT-STUDIO-01 — admin Shoot Studio: Pinterest inspo + selfie -> vault-anatomy photoshoot of Sandra (chat + tap cards, Maya pattern). Supersedes CONTENT-VISUALS-01 phases 1/2 (Sandra reviewed those: not usable, topic-first was inverted) | **Phase A live 2026-06-12** on /admin/content-brief. Phase B next: DB-backed vault + "publish as collection" (one approved shoot -> vault + freebie + studio vibe + Library drop, no deploy). Phase C: drop-email button. Spec: `tasks/SHOOT-STUDIO-01-admin-shoot-studio.md`. vault-prompt-writer skill recreated + COMMITTED at `.agents/skills/vault-prompt-writer/` (old one was a gitignored symlink, lost). Known gap to fix in B: `lib/app-v3/maya/vault-styles.ts` is a hard-coded 10-collection list; new collections get vibe tiles but NO Maya style DNA |
