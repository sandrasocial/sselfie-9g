...
| SHOOT-STUDIO-01 — inspiration-image photoshoots (now a collapsed support tool; engine + vault-anatomy prompt writer feed MAYA-ADMIN-01 slices) | Phase A live 2026-06-12. Phase B next: DB-backed vault + "publish as collection" (approved shoot -> vault + freebie + studio vibe + Library drop, no deploy). Phase C: drop-email button. Spec: `tasks/SHOOT-STUDIO-01-admin-shoot-studio.md`. vault-prompt-writer skill recreated + COMMITTED at `.agents/skills/vault-prompt-writer/`. Known gap to fix in B: `lib/app-v3/maya/vault-styles.ts` hard-coded 10-collection list = new collections get NO Maya style DNA |
