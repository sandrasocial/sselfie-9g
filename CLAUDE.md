...
| FUNNEL overhaul (8 moves, 2026-06-11) | **Built** — guide day-14 + new ai-prompts day-10 emails offer the trial; hourly `membership-checkout-recovery` cron (kill switch env `MEMBERSHIP_CHECKOUT_RECOVERY_DISABLED`); homepage repointed prompt-first; vault drops synced into Library (`scripts/sync-vault-drops.ts` — run after each new collection); trial day-0 email on claim. Win-back to 31 ex-members **SENT 2026-06-12** (27 delivered, 4 suppressed) |
| MAYA-ADMIN-01 — Maya IS the admin content surface (Sandra's call: don't build parallel tools, embed Maya + admin persona). Vault vibe tiles front and center: keep a collection's world, swap outfit/location in chat | **Slice 1 live 2026-06-12** on /admin/content-brief ("Content"): member Maya + `ADMIN_MAYA_CONTRACT` (distilled IG Growth OS), server-gated on admin email; weekly brief/shoot studio/carousel/story collapsed below as support tools. Next slices (one at a time): approve->vault publish, content tools in chat, admin memory, brief-as-knowledge. Spec: `tasks/MAYA-ADMIN-01-admin-maya.md` |
| SHOOT-STUDIO-01 — inspiration-image photoshoots (now a collapsed support tool; engine + vault-anatomy prompt writer feed MAYA-ADMIN-01 slices) | Phase A live 2026-06-12. Phase B built 2026-06-13: shoots now generate 6 shots by default, can be extended, require 6 approved rendered shots before publish, and publish into DB-backed `vault_collections`/`vault_prompts` so Vault, freebie previews, Maya vibe tiles/style guide, Library drops, and email-drop queue can read new collections without a per-collection deploy. Phase C next: explicit drop-email admin button with dry-run counts. Spec: `tasks/SHOOT-STUDIO-01-admin-shoot-studio.md`. vault-prompt-writer skill recreated + COMMITTED at `.agents/skills/vault-prompt-writer/`. |
| MEMBER-CHECKOUT-01 — membership checkout email capture (42 sessions/30d, 0 emails, recovery cron starving). Spec: `tasks/MEMBER-CHECKOUT-01-email-capture.md` | **Phase B ✅ DONE 2026-06-12**: Stripe products renamed to "SSELFIE SUITE"/"SSELFIE SUITE Annual", monthly now charges €97 EUR (`price_1ThYxHEVJvME7vkw32NBHPXB`, env repointed all Vercel envs, redeployed, verified live). Old USD price kept active for existing subs. **Phase A (email capture) ready for Codex** |
