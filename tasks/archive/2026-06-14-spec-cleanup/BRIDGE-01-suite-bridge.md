...
- **Phase C: BUILT 2026-06-11** — nav is now `Create · Photos · Content · Library · Account` (`app-v3-shell.tsx`): gallery tab relabeled "Photos" (eyebrow updated in `gallery-view.tsx`), new "Library" tab = `components/app-v3/library-view.tsx` (courses with lesson progress, owned products, weekly drops with empty state, locked previews + single upgrade CTA → `/join/studio?source=app_library` for non-members; members see everything open per D3). Data: new `app/api/app-v3/library/route.ts` (same auth pattern as gallery route; `getAcademyHomeState` + published `academy_monthly_drops` for members; membership locked-tile filtered out since the upgrade banner covers it). Account tab gained a "Your SSELFIE" pointer card into Library. Links point at existing `/academy/...` + `/access/...` routes (v3-native rendering = later iteration). Verified: tsc clean, route 401s unauthenticated, `/app` auth redirect intact, no new voice violations.
- **Phase D: BUILT 2026-06-11** — Migration applied to Neon + committed (`scripts/2026-06-11-suite-trial.sql`): `subscriptions.trial_ends_at TIMESTAMPTZ`, `(product_type, status)` index, `credit_transactions` CHECK extended with `trial_grant`/`trial_expiry`. New `lib/trial/suite-trial.ts` (grantSuiteTrial: one trial per user EVER, 20 credits via addCredits 'trial_grant'; getSuiteAccess → member/trial/limited/none; canGenerate). Claim flow `app/claim/[token]/page.tsx` (freebie_subscribers.access_token → find-or-create auth user → Neon user → grant → password-setup redirect or /app; invalid-token page). Trial unlock email appended to prompt-vault + starter-kit handlers (livemode only, email_logs idempotent, `suite_trial_unlock`). Gate in `app/app/page.tsx` now resolves via getSuiteAccess: member/trial → full app (trial banner + Account badge with days left, billing card hidden for trials), expired trial + one-time owners → limited mode (banner, Create tab locked behind upgrade card, MayaConcierge hidden), none → /studio (env flag behavior unchanged). Server-side generation lock in BOTH `maya/generate` and `maya/edit` routes (403 `generation_locked`, admin exempt). Expiry cron `app/api/cron/suite-trial-expiry` (registered in vercel.json, 08:45 UTC daily): day-5 "2 days left" email, flips overdue rows to 'expired', zeroes only the UNSPENT part of the 20-credit grant (usage-aware, top-ups never touched, 'trial_expiry' transaction), trial-ended email; all email_logs idempotent. Trial emails in `lib/email/templates/suite-trial.tsx` (Appendix 1.4/1.5/1.6 verbatim, stone shell, render-tested). Admin contract: `tests/suite-trial-contract.test.ts` pins suite_trial out of member counts + the server-side lock.
- **Phase E: BUILT 2026-06-11** — analytics_events (behavior only): `trial_claimed` (claim page), `trial_first_generation` (generate route, once per user), `trial_expired` (cron). Upgrade-click measurement intentionally uses checkout source attribution instead of an analytics event (sources: `trial_banner`, `trial_account`, `trial_day5`, `trial_ended`, `app_limited_create`, `app_library` → checkout_attribution is the contract-correct "where buyers came from" store). Trial → paid conversion on /admin home: `lib/admin/home-report.ts` gains a `trials` block (active/expired/converted, source: subscriptions, NO money fields) joined via later membership rows; rendered under the Paying members card in `app/admin/page.tsx` (only when trials exist).
- **Backfill broadcast: SENT 2026-06-11 (Sandra approved).** 26 past Vault/Kit buyers via `scripts/bridge-01-trial-backfill.ts` (backfill variant of the approved 1.4 copy). 20 delivered, 6 correctly suppressed (3 unsubscribed, 1 bounced, 2 suppression-listed). Claim flow was smoke-tested end-to-end against prod (disposable test user, fully cleaned up) before the send. Re-runs are safe (email_logs idempotent) but the send is one-time — do not re-run with --send unless Sandra asks.
- Remaining: video tile on /join/studio when VIDEO-01 ships. BRIDGE-01 is otherwise COMPLETE.
