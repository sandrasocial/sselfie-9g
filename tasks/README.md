...
| 5 | `CONTENT-CAROUSEL-03-image-model-as-designer.md` | Codex | **CORRECTION — pending Sandra's go before rebuild.** Image model IS the designer: each slide = gpt-image-2 from (reel screenshot + approved style-ref + optional selfie) → finished 1080x1350 PNG in Sandra's dark editorial style. Retire the white-card next/og renderer as default. **Style-reference library (31 approved slides) ingested by Claude** (`content_style_references`). Reel refs + grounding kept. Supersedes the 01/02 renderer-as-designer approach. |
