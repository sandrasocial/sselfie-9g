...
| 5 | `CONTENT-CAROUSEL-01-tutorial-carousel-mode.md` | Codex | **READY FOR CODEX** (Sandra approved 2026-06-15). Tutorial Carousel mode by EXTENDING content-kit (no parallel studio): tutorial template, before/after slide, burgundy callouts (ported from story renderer), hybrid render (gpt-image-2 bakes short text on scenes; real screenshots kept exact + composited callouts), Maya tool that reads `content_reel_references`. **Data layer (reel-reference extractor + table, 185 refs) DONE by Claude.** Best after FIX/GROUNDING land (shares content-kit files). Burgundy hex `#6E2A35` provisional, Sandra to confirm. Phase 2 (auto-batch/fal) scoped out. |
