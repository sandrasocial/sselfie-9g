...
| 5 (pending yes) | `CONTENT-CAROUSEL-03-image-model-as-designer.md` | Codex | Spec REWRITTEN to corrected scope 2026-06-15, pending Sandra's final yes. Carousel from a real reel's OWN frames: slide 1 = real result shot (untouched); step slides = real screenshots full-bleed + SSELFIE editorial overlay (serif + muted oxblood callouts) COMPOSITED on top (screenshot preserved, no generation, NOT white cards). All image generation (CAROUSEL-02 new-world) PARKED until coherent. Reuses next/og photo-background path + ported callout SVGs. |
