...
| 5 | `CONTENT-CAROUSEL-02-FIX-generation-inputs.md` | Codex | **READY** — post-review hardening (small). Generate new-world cover/result from admin selfies + world prompt ONLY; stop feeding the reel cover (baked text + old location) into gpt-image-2; code-enforce that no `content_reel_references` URL reaches generation (today it's label-enforced). Fixes ghost-text risk + makes the screenshot-safe invariant airtight. Flip the CAROUSEL-02 test assertion. |
