...
| 4 | `CONTENT-INTELLIGENCE-CLEANUP-01-phase2.md` | Codex | 2026-07-09: Phase 2 of the content-intelligence migration to Claude Cowork — do NOT start until the weekly-content-brief-draft and ig-dm-drafter Cowork tasks have run successfully at least once each. Deletes the dead Admin Maya brain, the content-brief-weekly/content-brief-jobs crons + CONTENT_BRIEF_ENABLED, daily-briefing-intelligence.ts, lib/admin/post-now.ts; collapses /admin nav to Home/Inbox/Content/Support/Tools; updates every stale hardcoded model ID to the current Sonnet 5/Opus 4.8 family (verify exact slugs at build time, do not copy old strings). Spec to be written once Phase 1 is proven. |
