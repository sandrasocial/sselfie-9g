...
async function main() {
  ...
  if (newSubs.ok) {
    lines.push(
      `- New subscriptions: ${newSubs.value.count} (active: ${newSubs.value.active}, past_due: ${newSubs.value.past_due}, linked_to_stripe_sub: ${newSubs.value.linked_to_stripe_sub}) (last: ${newSubs.value.last_created ? iso(newSubs.value.last_created) : "n/a"})`,
    )
  } else {
  ...
  if (payments.ok) {
    lines.push(
      `- Stripe payments (succeeded): ${payments.value.count} (subscription payments: ${payments.value.subscription_payment_count}, sum_cents: ${payments.value.amount_cents_sum ?? 0}) (last: ${payments.value.last_created ? iso(payments.value.last_created) : "n/a"})`,
    )
  } else {
  ...
  if (freebieBonus.ok) {
    lines.push(
      `- Welcome bonus grants: ${freebieBonus.value.count} tx to ${freebieBonus.value.users_count} user(s), credits: ${freebieBonus.value.credits_sum ?? 0} (last: ${freebieBonus.value.last_created ? iso(freebieBonus.value.last_created) : "n/a"})`,
    )
  } else {
  ...
  if (freebieUsage.ok) {
    lines.push(
      `- Bonus users who spent credits: ${freebieUsage.value.users_who_spent}/${freebieUsage.value.bonus_users} (${pct(Number(freebieUsage.value.users_who_spent ?? 0), Number(freebieUsage.value.bonus_users ?? 0))}) (spend events: ${freebieUsage.value.spend_events}, credits_spent: ${freebieUsage.value.credits_spent ?? 0})`,
    )
  } else {
  ...
  if (activation.ok) {
    lines.push(
      `- First output activation (new users in window): ${activation.value.activated_any_time}/${activation.value.signups_in_window} (${pct(Number(activation.value.activated_any_time ?? 0), Number(activation.value.signups_in_window ?? 0))})`,
    )
    const maturedSignups = Number(activation.value.matured_signups ?? 0)
    const d1ActivationLabel =
      maturedSignups > 0
        ? `${activation.value.activated_within_24h}/${activation.value.matured_signups} (${pct(Number(activation.value.activated_within_24h ?? 0), maturedSignups)})`
  ...
  if (paidCreditPurchases.ok) {
    lines.push(
      `- Paid credit purchases: ${paidCreditPurchases.value.count} (linked stripe_payment_id: ${paidCreditPurchases.value.linked_count}, payment_sum_cents: ${paidCreditPurchases.value.payment_amount_cents_sum ?? 0}, credits_granted: ${paidCreditPurchases.value.credits_sum ?? 0}) (last: ${paidCreditPurchases.value.last_created ? iso(paidCreditPurchases.value.last_created) : "n/a"})`,
    )
  } else {
