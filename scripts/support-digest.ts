...
async function main() {
  ...
  const realNewUsers = (newUsers as any[]).filter((user) => !isSyntheticAnalyticsEmail(user.email))
  const syntheticNewUsers = (newUsers as any[]).filter((user) => isSyntheticAnalyticsEmail(user.email))
